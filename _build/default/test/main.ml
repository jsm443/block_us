...
(* In our testing, we focus heavily on testing the placement checks
   ...
   working, and we tested the top-level function valid_placement in
   order to make sure it was properly calling the checks. We also
   included test cases to check our commands to ensure the user side was
   working as well. Further, we included test cases for getting piece
   coordinates to make sure our pieces were constructed correctly. Test
   cases were developed using both black and glass box testing. When
   using glass box testing, we looked at the actual code for our checks
   and tested different paths in our code. For example, when testing
   check valid first move, we look and saw that there was an if
   statement so we chose to test the 2 possible branches, whether you
   try to piece first piece on a corner or not. We also implemented
   black box testing by making test cases based off of edge cases in the
   specification. For example, when checking overlapping pieces, we
   wanted to check the case when not overlapping, overlapping with piece
   of same color, and overlapping with piece of other player's color. We
   followed the same logic of glass box and black box testing for more
   complicated functions in order to find any errors. Further, after
   every feature addition, we manually tested our game by playing it and
   seeing if the feature was implemented correctly. We added print
   statements for illegal moves and made sure they would pop up if we
   entered a wrong move. So, our test suite demonstrates correctness of
   our program because by looking at both the code, specifications, and
   by manually testing the game we were able to test all of the edge
   cases and make sure our tests were comprehensive. *)
